...
require_once("../database/PHPMailer/mail.php");
session_start();
...
sendEmail($row['email'], $row['nome'], $row['verify_cod']);
?>
...
<body>
  ...
  <form action="../database/database.php" method="POST" class="container min-w-full min-h-screen grid">
    <input type="hiden" name="acao" value="esquecisenha" class="hidden">
    <div
      class="login p-4 h-96 w-80 self-center justify-self-center shadow-lg shadow-gray-400/40 rounded-lg grid grid-cols-1 gap-3 grid-rows-4">
      <div class="Code">
        <label for="code">Código recebido por email</label> <br />
        <div class="relative">
      ...
      <button class="text-xl rounded-lg bg-sky-500 transition duration-700 ease-in-out hover:bg-sky-400" id="changePsw">
        Alterar Senha
      </button>
